...
export type MetrikaEvent = string

export class MetrikaLoggerAdapter implements LoggerAdapter {
  ...
  private pointYandexGoal(event: MetrikaEvent, payload?: Record<string, any>): void {
    this.logEvent('GOAL', event, payload)

    if (this.isEnabled) {
      try {
        this.metrikaService.reachGoal(event, payload)
      } catch (error) {
  ...
  private fireAnalyticsEvent(event: MetrikaEvent, payload?: Record<string, any>): void {
    this.logEvent('EVENT', event, payload)
  ...
  trackEvent(event: MetrikaEvent, payload?: Record<string, any>): void {
    this.pointYandexGoal(event, payload)
    this.fireAnalyticsEvent(event, payload)
  }
}
